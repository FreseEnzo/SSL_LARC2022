...
public:
     ...
     */
    SX1280Hal( SPI_HandleTypeDef* hspi,
...
protected:
    ...
    SPI_HandleTypeDef* RadioSpi;                    //!< The SPI object used to communicate with the radio
    GPIO_TypeDef* RadioNssPort;						//!< The pin connected to Radio chip select (active low)
    uint16_t RadioNssPin;
    GPIO_TypeDef* RadioResetPort;					//!< The reset pin connected to the radio
    uint16_t RadioResetPin;
    GPIO_TypeDef* RadioCtsnPort;					//!< The Clear To Send radio pin (active low)
    uint16_t RadioCtsnPin;
